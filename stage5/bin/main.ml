...
let parse_move_string ms =
  ...
  let c, f, t = (nth_exn words 1), (nth_exn words 3), (nth_exn words 5)
  in
...
let tops = List.init (List.length stacks) ~f:(fun i -> Stack.pop_exn (List.nth_exn stacks i))
